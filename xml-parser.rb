			...
			tag.xpath("level2").each do |article|
				...
				article.xpath("level3").each do |chapter|
					...
					chapter.xpath("section").each do |section|
						...
						o.write("<unit label='chapter' identifier='#{part_count.to_s+'.'+article_count.to_s}' order_by='#{part_count.to_s+'.'+article_count.to_s}' level='#{(depth-2).to_s}'>#{article.xpath('breadcrumbs/crumb[3]/caption')[0].content.strip}</unit>\n")
						o.write("<unit label='article' identifier='#{part_count.to_s+'.'+article_count.to_s+'.'+chapter_count.to_s}' order_by='#{part_count.to_s+'.'+article_count.to_s+'.'+chapter_count.to_s}' level='#{(depth-1).to_s}'>#{chapter.xpath('breadcrumbs/crumb[4]/caption')[0].content.strip}</unit>\n")
						o.write("</structure>\n")
